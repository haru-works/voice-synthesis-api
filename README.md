# Voice Synthesis API for Discord TTS Bot

複数のTTS（Text-to-Speech）エンジンを統一されたインターフェースで利用するためのDiscord読み上げBot用ラッパーAPIです。Honoフレームワークをベースに構築しています。APIキー認証、複数エンジンによる話者の負荷分散（シャーディング）機能を提供します。
